...
from configs import retrieve_dict_config, ObjectDict, retrieve_config as config, topic_link_res_path, sparse_methods, check_retrieve_config
...
def load_embedding_model():
    ...
    global wiki2vec
    wiki2vec_pkl_path = '/home/xxx/code/erm/data/wiki2vec/100d/enwiki_20180420_100d.pkl' if config.dim == 100 else \
        '/home/xxx/code/erm/data/wiki2vec/500d/enwiki_20180420_500d.pkl'
    ...
    if config.embedding_method == 'kgtk':
        ...
        if config.embedding_method == 'dbpedia_rdf2vec_sg_500': model_path = '/home/xxx/code/erm/data/rdf2vec/dbpedia/DB2Vec_sg_500_5_5_15_4_500'
        elif config.embedding_method == 'dbpedia_rdf2vec_sg_200': model_path = "/home/xxx/code/erm/data/rdf2vec/dbpedia/dbpedia_500_4_sg_100"
        elif config.embedding_method == 'wikidata_rdf2vec_sg_200_1': model_path = "/home/xxx/code/erm/data/rdf2vec/wikidata/wikid2Vec_sg_200_5_5_15_4_500"
        elif config.embedding_method == 'wikidata_rdf2vec_sg_200_2': model_path = "/home/xxx/code/erm/data/rdf2vec/wikidata/wikid2Vec_sg_200_7_4_15_4_500"
        else:
...
def init_pair2score():
    cursor.execute(f'SELECT query_entity_uri, dataset_entity_uri, text_similarity FROM `query_dataset_entity_similartity_kgtk`')
    for (query_entity_uri, dataset_entity_uri, text_similarity) in cursor.fetchall():
        if text_similarity == -1: continue
...
def get_all_dids():
    ...
    if test_collection_name == 'BDR':
        # cursor.execute(f'select distinct(dataset_id) from {test_collection_name}_candidate_metadata')
        pooling_path = '/home/xxx/code/reproduce_keds/data/retrieve_results/BDR/candidates/pooling_dataset_ids.json'
        dids = json_load(pooling_path)
    else:
        cursor.execute(f'select distinct(dataset_id) from {test_collection_name}_metadata_info')
        dids = [int(did) for (did, ) in cursor.fetchall()]
    return dids
...
def init_entity_and_word_vectors():
    ...
    global q_vectors_total, d_vectors_total, dids
    ...
    if link_method == "TAGME":
        ...
        for (qid, wiki_title, confidence, spot) in cursor.fetchall():
        ...
        cursor.execute(sql)
        log.info('load query_entity_info')
        for (qid, spot, wiki_title, rank) in cursor.fetchall():
        ...
        cursor.execute(sql)
        log.info('load query_entity_info')
        for (qid, spot, wiki_title) in cursor.fetchall():
            # log.info(f'{qid} {spot} {wiki_title} {rank}')
            if qid not in query_entity_info.keys(): query_entity_info[qid] = []
            confidence = 1
            query_entity_info[qid].append((spot, wiki_title, confidence))

    elif link_method=='NPR':
        if config.test_collection_name == 'BDR':
            table_name = f'{config.test_collection_name}_target_metadata_NPR'
            sql = f'SELECT dataset_id, text, entity_id, similarity, edit_dist, abbreviation from {table_name} where entity_id is not NULL;'
        else:
            table_name = f'{config.test_collection_name}_query_NPR'
            sql = f'SELECT query_id, text, entity_id, similarity, edit_dist, abbreviation from {table_name} where entity_id is not NULL;'
        cursor.execute(sql)
        log.info('load query_entity_info')
        for (qid, text, wiki_title, similarity, edit_dist, abbreviation) in cursor.fetchall():
            if similarity is None or edit_dist is None or abbreviation is None: continue
            if similarity < 0.9 and edit_dist >= 10 and abbreviation == 0: continue
            qid = str(qid)
            if qid not in query_entity_info.keys(): query_entity_info[qid] = []
            confidence = 1
            query_entity_info[qid].append((text, 'http://www.wikidata.org/entity/' + wiki_title, confidence))
    elif link_method == 'ReFinED':
        table_name = f'{config.test_collection_name}_query_link_result_ReFinED'
        sql = f'SELECT query_id, span_text, entity from {table_name} WHERE entity is not NULL;'
        cursor.execute(sql)
        log.info('load query_entity_info')
        for (qid, spot, wiki_title) in cursor.fetchall():
            qid = str(qid)
            if qid not in query_entity_info.keys(): query_entity_info[qid] = []
            confidence = 1
            query_entity_info[qid].append((spot, 'http://www.wikidata.org/entity/' + wiki_title, confidence))
    ...
    if config.link_method == 'TAGME':
        table_name = f'{test_collection_name}_metadata_link_result_{config.link_method}'
        sql = f'SELECT dataset_id, wiki_title, {confidence_name}, spot from {table_name} {sql_where}'
        cursor.execute(sql)
        for (did, wiki_title, confidence, spot) in cursor.fetchall():
        ...
        cursor.execute(sql)
        for (did, spot, wiki_title, rank) in cursor.fetchall():
            if did not in dataset_entity_info.keys(): dataset_entity_info[did] = []
            confidence = 1
            dataset_entity_info[did].append((spot, wiki_title, confidence))
        log.info('load dataset_entity_info done')
        ...
        # load entity info
        cursor.execute(sql)
        for (did, spot, wiki_title) in cursor.fetchall():
            if did not in dataset_entity_info.keys(): dataset_entity_info[did] = []
            confidence = 1
            dataset_entity_info[did].append((spot, wiki_title, confidence))
    elif link_method=='NPR':
        if config.test_collection_name == 'BDR':
            table_name = f'{test_collection_name}_candidate_metadata_NPR'
            str_dids = ','.join([str(did) for did in dids])
            sql = f'SELECT dataset_id, text, entity_id, similarity, edit_dist, abbreviation FROM {table_name} where entity_id is not NULL and dataset_id in ({str_dids});'
        else:
            table_name = f'{test_collection_name}_{data_field}_NPR'
            sql = f'SELECT dataset_id, text, entity_id, similarity, edit_dist, abbreviation FROM {table_name} where entity_id is not NULL;'
        cursor.execute(sql)
        for (did, text, wiki_title, similarity, edit_dist, abbreviation) in cursor.fetchall():
            if similarity is None or edit_dist is None or abbreviation is None: continue
            if similarity < 0.9 and edit_dist >= 10 and abbreviation == 0: continue
            did = int(did)
            if did not in dataset_entity_info.keys(): dataset_entity_info[did] = []
            confidence = 1
            dataset_entity_info[did].append((text, 'http://www.wikidata.org/entity/' + wiki_title, confidence))
        log.info('load dataset_entity_info done')
    elif link_method == 'ReFinED':
        table_name = f'{test_collection_name}_metadata_link_result_ReFinED'
        sql = f'SELECT dataset_id, span_text, entity from {table_name} WHERE entity is not NULL;'
        cursor.execute(sql)
        for (did, spot, wiki_title) in cursor.fetchall():
            did = int(did)
            if did not in dataset_entity_info.keys(): dataset_entity_info[did] = []
            confidence = 1
            dataset_entity_info[did].append((spot, 'http://www.wikidata.org/entity/' + wiki_title, confidence))
        log.info('load dataset_entity_info done')

    # print(dataset_entity_info)
    # print(query_entity_info)
...
def rerank_start(res_dir_name = 'single_run', w1 = 0.5):
    sparse_candidate_dir_path = f'/home/xxx/code/erm/data/retrieve_results/{config.test_collection_name}/candidates'
    # sparse_candidate_dir_path = f'/home/xxx/code/reproduce_keds/data/retrieve_results/{config.test_collection_name}/candidates'

    retrieve_res_dir_path = f'/home/xxx/code/reproduce_keds/data/retrieve_results/{config.test_collection_name}/{config.stg}/{config.embedding_method}/{config.data_field}/{res_dir_name}/'
    ...
    eval_keds(retrieve_res_dir_path)
    ...
    candidate_merge_op = [7]
        
    for method in sparse_methods:

        sparse_res_path = f'/home/xxx/code/erm/data/retrieve_results/{config.test_collection_name}/candidates/{method} {config.train_or_test}_top100_sorted.json'
    ...
    get_path_and_merge(paths_and_op_list, w1 = w1)
